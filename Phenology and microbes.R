...
micrtax <- gsub("mixed","Mixed",wos_rel$micrtax)
micrtax <- gsub("bacteria","Bacteria", micrtax)
micrtax <- gsub("fungi","Fungi", micrtax)
micrtax <- gsub("virus","Virus",micrtax)

micrloc <- gsub("shoot","Shoot",wos_rel$micrloc)
micrloc <- gsub("root","Root",micrloc)
micrloc <- gsub("reproductive","Reproductive",micrloc)
micrloc <- gsub("seed","Seed",micrloc)

phentrait <- gsub("budburst","Budburst",wos_rel$phentrait_of)
phentrait <- gsub("f","F", phentrait)
phentrait <- gsub("other Flo","Other flo", phentrait)
phentrait <- gsub("germ","Germ", phentrait)
phentrait <- gsub("phyllochron","Phyllochron", phentrait)
phentrait <- gsub("senescence time","Senescence time", phentrait)
phentrait <- gsub("mat","Mat", phentrait)


taxand <- gsub("; ", micrtax,replacement="&")
locand <- gsub("; ", micrloc,replacement="&")
phenand <- gsub("; ", phentrait,replacement="&")
...
#Appendix figures
pdf("upsetrtax.pdf",width=5.5,height=5)
upset(fromExpression(table(taxand)),nsets=15, order.by = "freq", mainbar.y.max = 45,
	mainbar.y.label="No. with microbe taxonomy intersect", sets.x.label = "No. with microbe taxonomy")
dev.off()
pdf("upsetrloc.pdf",width=5,height=5)
upset(fromExpression(table(locand)),nsets=10, order.by = "freq", mainbar.y.max=105,
	mainbar.y.label="No. with microbes in tissue/s in intersect", sets.x.label = "No. with microbes in tissue")
dev.off()
...
plot(c(0,1)~I(c(0,1)),pch=NA,xlab="",ylab="",bty="n",xaxt="n",ylim=c(-0.3,1.2),yaxt="n")
	...
	text(xvals,y=1.2,siggroupsFE)
	text(xvals,y=-0.2,siggroupsFD,col="gray")
	mtext(side=3,"Flowering time",adj=0.6)
	axis(side=2,at=c(0,0.25,0.5,0.75,1),labels=c(0," ",0.5, " ",1))
legend(xvals[5]-0.05,y=1.2,c("Earlier","Delay"),fill=c("black","gray"),bty="n")
plot(c(0,1)~I(c(0,1)),pch=NA,xlab="",ylab="",bty="n",xaxt="n",ylim=c(-0.3,1.2),yaxt="n")
	...
	text(xvals,y=1.2,siggroupsGtE)
	text(xvals,y=-0.2,siggroupsGtD,col="gray")
	mtext(side=3,"Germination time",adj=0.6)
	mtext(side=2,"Probability of observing effect",line=2)
	axis(side=2,at=c(0,0.25,0.5,0.75,1),labels=c(0," ",0.5, " ",1))
legend(xvals[5]-0.05,y=1.2,c("Earlier","Delay"),fill=c("black","gray"),bty="n")
plot(c(0,1)~I(c(0,1)),pch=NA,xlab="",ylab="",bty="n",xaxt="n",ylim=c(-0.3,1.2),yaxt="n")
	...
	text(xvals,y=1.2,siggroupsGpE)
	text(xvals,y=-0.2,siggroupsGpD,col="gray")
	mtext(side=3,"Germination probability",adj=0.7)
	axis(side=2,at=c(0,0.25,0.5,0.75,1),labels=c(0," ",0.5, " ",1))
legend(xvals[4]+0.05,y=1.2,c("Narrowed","Expanded"),fill=c("black","gray"),bty="n")
axis(side=1,at=xvals, labels=xlab,las=2)
...
xlab <- c("Budburst","Floral budset time","Flowering time","Fruiting time","Germination time","Phyllochron" )#sort(unique(allphen$phentrait))
par(mar=c(1,0,1,0))
par(oma=c(7,4,1,1))
plot(c(0,1)~I(c(0,1)),pch=NA,xlab="",ylab="",bty="n",xaxt="n",ylim=c(-0.3,1.2),yaxt="n")
